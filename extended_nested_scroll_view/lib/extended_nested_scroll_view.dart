...
class _NestedScrollCoordinator
  ...
  Iterable<_NestedScrollPosition> get _innerPositions {
    if (innerScrollPositionKeyBuilder != null &&
        _innerController.nestedPositions.isNotEmpty) {
      var key = innerScrollPositionKeyBuilder();
      var list = _innerController.nestedPositions.where((x) {
        return x.key == key;
      });
      //sometime in _innerController attach postion method it will get wrong key
      if (list.isEmpty) {
        var hasTheSameKey = false;
        List<String> listKey = List<String>();
        _innerController.nestedPositions.forEach((f) {
          listKey.add(f.key?.toString());
        });

        hasTheSameKey = listKey.length != Set.from(listKey).length;
        if (hasTheSameKey) {
          _innerController.resetScrollPositionKey();
          list = _innerController.nestedPositions.where((x) {
            return x.key == key;
          });
          print("hasTheSameKey");
        }
...
class _NestedScrollController extends ScrollController {
  ...
  void attach(ScrollPosition position) {
    ...
    if (debugLabel == "inner" && position is _NestedScrollPosition) {
      setScrollPositionKey(position);
    }
  }

  void setScrollPositionKey(_NestedScrollPosition position) {
    final type = _typeOf<NestedScrollViewInnerScrollPositionKeyWidget>();
    NestedScrollViewInnerScrollPositionKeyWidget keyWidget =
        (position.context as ScrollableState)
            .context
            .ancestorWidgetOfExactType(type);
    position.key = keyWidget?.scrollPositionKey;
    print(position.key.toString());
  }

  void resetScrollPositionKey() {
    nestedPositions.forEach((p) {
      setScrollPositionKey(p);
    });
  }
...
//    if (debugLabel == "inner") {
//      setScrollPositionKey();
//    }
    return super.applyContentDimensions(minScrollExtent, maxScrollExtent);
  }

  void setScrollPositionKey() {
    final type = _typeOf<NestedScrollViewInnerScrollPositionKeyWidget>();
    NestedScrollViewInnerScrollPositionKeyWidget keyWidget =
        (this.context as ScrollableState)
            .context
            .ancestorWidgetOfExactType(type);
    key = keyWidget?.scrollPositionKey;
    print(key?.toString());
  }
